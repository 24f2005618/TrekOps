...
export default {
  data(){
    ...
    created(){
        fetch(import.meta.env.VITE_SERVER+"/admin/getRoute/"+this.$route.params.id,{
            method:"GET",
            headers:{
            ...
            if(r.status == 200){
                r.json().then(x => {
                    ...
                    this.form.difficulty = x.difficulty;
                    this.form.coordinates = x.coordinates;
	...
	methods:{
		...
		editRoute(){
			if(!this.validateName(this.form.name)){
				...
				fetch(import.meta.env.VITE_SERVER+"/admin/editRoute/"+this.$route.params.id,{
						method:"PATCH",
						headers:{
								"Authentication-Token":this.$store.state.user.token
						},
